...
async def game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    empty_cells = [(r, c) for r in range(3) for c in range(3) if board[r][c] == FREE_SPACE]
    if empty_cells:
...
def won(board: List[List[str]]) -> bool:
    ...
    # Check diagonals
    if board[0][0] == board[1][1] == board[2][2] != FREE_SPACE or \
       board[0][2] == board[1][1] == board[2][0] != FREE_SPACE:
        return True
